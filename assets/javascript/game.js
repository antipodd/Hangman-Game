...
var myAudio = new Audio('./assets/sounds/SuperMarioBros.mp3'); 
...
document.onkeyup = function(event) {
  if (/*myAudio.duration > 0 &&*/ !myAudio.paused) { // need this within document.onkeyup to prevent keyup events from doing anything during setTimeout
    if ((event.keyCode >= 65 && event.keyCode <= 90) || (event.keyCode >= 97 && event.keyCode <= 122)) {    
      ...
      if (display.join('') === word) {
        myAudio.pause();
        playWinningSound();
          //console.log(document.getElementById('win').play());
        ...
        document.querySelector('.win-no').innerHTML = wins;
          ...
          /*var b = document.querySelector('.win-no');
